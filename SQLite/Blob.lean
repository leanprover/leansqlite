...
public instance : ToBinary Empty where
  serializer := nofun

public instance : FromBinary Empty where
  deserializer := throw "Can't deserialize `Empty`"
...
public instance [ToBinary α] [ToBinary β] : ToBinary (α ⊕ β) where
  serializer
    | .inl x, b => b |>.push 0 |> ToBinary.serializer x
    | .inr y, b => b |>.push 1 |> ToBinary.serializer y

public instance [FromBinary α] [FromBinary β] : FromBinary (α ⊕ β) where
  deserializer := do
    match (← .byte) with
    | 0 => .inl <$> FromBinary.deserializer
    | 1 => .inr <$> FromBinary.deserializer
    | other => throw s!"Expected 0...2 for `Sum`, got {other}"
...
public instance : ToBinary Lean.Name where
  serializer := go
where go
  | .anonymous, b => b.push 0
  | .num n i, b => b |>.push 1 |> go n |> ToBinary.serializer i
  | .str n s, b => b |>.push 2 |> go n |> ToBinary.serializer s

public partial instance : FromBinary Lean.Name where
  deserializer := go
where
  go := do
    match (← .byte) with
    | 0 => return .anonymous
    | 1 => .num <$> go <*> FromBinary.deserializer
    | 2 => .str <$> go <*> FromBinary.deserializer
    | other => throw s!"Expected tag 0...3 for `Name`, got {other}"
